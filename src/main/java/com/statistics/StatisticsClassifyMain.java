...
public class StatisticsClassifyMain {
	...
	public static void statisticsAverage() throws SQLException{
		...
		for(int i = 0; i < areaNameList.length; i++){
			...
			List<RatingList> list = sqlSession.selectList("selectRatingListByClassifyMain", areaNameList[i]);
			int reviewCount5 = 0;
			int reviewCount4 = 0;
			int reviewCount3 = 0;
			int reviewCount2 = 0;
			...
			System.out.println(areaNameList[i] + "的" + list.size() + "条评分记录,平均数=" + avgRatingValue + ",方差=" + varianceRatingValue);
			...
			String sqlString = "UPDATE classifymainstatistics SET reviewCount5=" + reviewCount5 + 
					...
					" where classifyMain='" + areaNameList[i] + "';";
